...
(*
  2020/1/1 Hisabumi Hatsugai
  'enum_lnr', 'enum_rnl' is added.
*)
...
module type S =
  sig
    ...
    val enum_lnr : 'c -> 'a t -> ('c -> (key * 'a) option -> ('c -> unit) -> unit) -> unit
    val enum_rnl : 'c -> 'a t -> ('c -> (key * 'a) option -> ('c -> unit) -> unit) -> unit
...
module Make(Ord: OrderedType) = struct
    ...
    let enum_lnr c n f =
      ...
      and g c ps =
        match ps with
          [] -> f c None (fun _ -> ())
        | (x, p)::ps -> f c (Some x) (fun c -> h c p ps)
      in h c n []
